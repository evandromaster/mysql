...
CREATE PROCEDURE TESTE6 (inicio char(30), fim char(30), CiaBase char(10))
BEGIN
	
    set @inicio = str_to_date(inicio, '%d/%m/%Y');
    set @fim = str_to_date(fim, '%d/%m/%Y');

	select NREDS, COD_DIAO, BAIRRO, BAIRRO_NAO_CAD, LOG_TIPO, LOGRADOURO, LOGRADOURO_NAO_CAD, COMPLEMENTO_END, PONTO_REF, UN_AREA_MIL, latitude, longitude,
case -- case 1
when
					...
					when	
						...
						when	
							...
							when	
								...
								case -- case 9
								when
									COMPLEMENTO_END in (SELECT VALIDADOR FROM tbl_validador_cia WHERE CIA_BASE = '53ª CIA PM' and VALIDADOR_TIPO = 'COMPL_ENDERECO') then '53 Cia'
								when
                                    COMPLEMENTO_END in (SELECT VALIDADOR FROM tbl_validador_cia WHERE CIA_BASE = '139ª CIA PM' and VALIDADOR_TIPO = 'COMPL_ENDERECO') then '139 Cia'
								when	
                                    COMPLEMENTO_END in (SELECT VALIDADOR FROM tbl_validador_cia WHERE CIA_BASE = '142ª CIA PM' and VALIDADOR_TIPO = 'COMPL_ENDERECO') then '142 Cia'
					...
					when	
						...
						when	
							...
							when	
								...
								case -- case 9
								when
									COMPLEMENTO_END in (SELECT VALIDADOR FROM tbl_validador_cia WHERE CIA_BASE = '53ª CIA PM' and VALIDADOR_TIPO = 'COMPL_ENDERECO') then 'COMPLEMENTO_END'
								when
                                    COMPLEMENTO_END in (SELECT VALIDADOR FROM tbl_validador_cia WHERE CIA_BASE = '139ª CIA PM' and VALIDADOR_TIPO = 'COMPL_ENDERECO') then 'COMPLEMENTO_END'
								when	
                                    COMPLEMENTO_END in (SELECT VALIDADOR FROM tbl_validador_cia WHERE CIA_BASE = '142ª CIA PM' and VALIDADOR_TIPO = 'COMPL_ENDERECO') then 'COMPLEMENTO_END'
...
(DATA_FATO between @inicio and @fim)
and
...
CALL TESTE6('01/01/2018', '10/01/2018', '%53%');