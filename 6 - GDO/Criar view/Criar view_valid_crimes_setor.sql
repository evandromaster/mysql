drop view if exists vw_valid_crimes_setor;
...
or ( -- INÍCIO CONDIÇÃO NATUREZA SECUNDARIA2		
				NREDS in (select NREDS from tbl_controle_auditorias where INDICADOR = 'INCLUSAO CRIME VIOLENTO')  
			) -- FIM CONDIÇÃO 2 (NATUREZAS)
...
case -- condições para crimes TRI
	when 	(	
				...
				and timediff(concat(DATA_COM, ' ', HORA_COM), concat(DATA_FATO, ' ', HORA_FATO)) < convert('06:00:00', time)            
...
-- end as MESANTERIOR,
	...
	case
	when NREDS in (select NREDS from tbl_controle_auditorias where INDICADOR = 'INCLUSAO CRIME VIOLENTO')	then 'C01157' -- ROUBO
	else
		case
...
-- 'cr' as rel
from tbl_crimes
where NREDS not in (select NREDS from tbl_controle_auditorias where INDICADOR IN ('DESCONTABILIZAR REDS', 'CONFLITO DE ENDEREÇO') AND AUDITADO = 'SIM')
;